...
class BusinessRules @Inject()() {
    ...
    (baReport \\ "TypeOfTax" \ "_").headOption match {
      case Some(node) => node.label match {
        ...
        case _ =>
          ...
          Error(ErrorCode.UNSUPPORTED_TAX_TYPE, Seq(s"${node.label}")) :: List()
      }
      case None =>
        Logger.warn("Xml element not found: TypeOfTax")
        Error(ErrorCode.UNKNOWN_TYPE_OF_TAX, Seq()) :: List()
    }
  }
  ...
  private def validateCTaxCode(implicit node:NodeSeq): List[Error] = {
    ...
    repCode match {
      case "CR03" => // no existing- only 1 proposed
        if (proposedEntries != 1) lb += Error(ErrorCode.ONE_PROPOSED, Seq(s"$reportNumber",s"$repCode"))
        if (existingEntries != 0) lb += Error(ErrorCode.NONE_EXISTING, Seq(s"$reportNumber",s"$repCode"))
      case "CR04" => // either 1 existing or 1 proposed
        if (((proposedEntries == 1) && (existingEntries ==0 )) ||
          ((proposedEntries == 0) && (existingEntries == 1))){}
        else
          lb += Error(ErrorCode.EITHER_ONE_EXISTING_OR_ONE_PROPOSED,Seq(s"$reportNumber", s"$repCode"))
      case "CR05" => // at least 1 existing and at least 1 proposed
        if (proposedEntries == 0) lb += Error(ErrorCode.ATLEAST_ONE_PROPOSED, Seq(s"$reportNumber", s"$repCode"))
        if (existingEntries == 0) lb += Error(ErrorCode.ATLEAST_ONE_EXISTING, Seq(s"$reportNumber", s"$repCode"))
      case "CR08" => lb += Error(ErrorCode.NOT_IN_USE, Seq(s"$reportNumber", s"$repCode"))
      case "CR11" => lb += Error(ErrorCode.NOT_IN_USE, Seq(s"$reportNumber", s"$repCode"))
      case "CR12" => // 1 existing and 1 proposed
        if (proposedEntries != 1) lb += Error(ErrorCode.ONE_PROPOSED, Seq(s"$reportNumber", s"$repCode"))
        if (existingEntries != 1) lb += Error(ErrorCode.ONE_EXISTING, Seq(s"$reportNumber", s"$repCode"))
      case "CR13" => lb += Error(ErrorCode.NOT_IN_USE, Seq(s"$reportNumber", s"$repCode"))

      case _ => // default - 1 existing and none proposed
        if (proposedEntries != 0) lb += Error(ErrorCode.NONE_PROPOSED, Seq(s"$reportNumber", s"$repCode"))
        if (existingEntries != 1) lb += Error(ErrorCode.ONE_EXISTING, Seq(s"$reportNumber", s"$repCode"))
    }
    lb.toList
  }
  ...
  def baIdentityCodeErrors(xml:Node)(implicit request:Request[_]): List[Error] = {
    ...
    val codeInReport:String = (xml \\ "BillingAuthorityIdentityCode").text
      if (codeInReport.isEmpty) lb += ErrorCode.BA_CODE_REPORT
        request.headers.get("BA-Code") match {
          case Some(code) => if (codeInReport.length > 0 && code != codeInReport) lb +=
            ErrorCode.BA_CODE_MATCH
          case None => lb += ErrorCode.BA_CODE_REQHDR
        }
