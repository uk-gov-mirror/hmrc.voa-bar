...
import uk.gov.hmrc.voabar.util.ErrorCode.BA_CODE_MATCH
  ...
  "Validation service" must {
    ...
    "return a list of 2 errors when the BACode in the report header does " +
      ...
      validationService("0000").xmlNodeValidation(invalidBatch.head) mustBe List[Error](
        ...
        Error(ErrorCode.INVALID_XML_XSD, Seq("Invalid content was found starting with element 'BadElement'. " +
    ...
    "return a list of 3 errors when the BACode in the report batch header of a batch of 4 reports does " +
      ...
      val invalidBatch = reportBuilder.invalidateBatch(validBatch.head, Map(
       ...
       validationService("0000").xmlNodeValidation(invalidBatch.head) mustBe List[Error](
         Error(BA_CODE_MATCH, Seq()),
         Error(ErrorCode.INVALID_XML_XSD, Seq("Invalid content was found starting with element 'IllegalElement'. " +
           "One of '{\"http://www.govtalk.gov.uk/LG/Valuebill\":BillingAuthority}' is expected.")),
         Error(ErrorCode.INVALID_XML_XSD, Seq("Invalid content was found starting with element 'WrongElement'. " +
    ...
    "return a list of 3 errors when the BACode in the report batch header of a batch of 4 reports does " +
      ...
      validationService("0000").xmlNodeValidation(invalidBatch.head) mustBe List[Error](
        ...
        Error(ErrorCode.INVALID_XML_XSD,Seq("Invalid content was found starting with element 'BadElement'. " +
          "One of '{\"http://www.govtalk.gov.uk/LG/Valuebill\":PrimaryDescriptionCode, \"http://www.govtalk." +
          "gov.uk/LG/Valuebill\":SecondaryDescriptionCode, \"http://www.govtalk.gov.uk/LG/Valuebill\":PropertyDescriptionText}' is expected.")),
        Error(ErrorCode.CHARACTER,Seq("Header","IllegalElement","Some Valid Council"))
      )
    }
    ...
    "return a list of 1 error when a batch containing 1 report has an illegal char within the property report" in {
      ...
      validationService("9999").xmlNodeValidation(invalidBatch.head) mustBe List[Error](
        Error(ErrorCode.CHARACTER,Seq("211909","PersonGivenName","name"))
      )
    }
    ...
    "return a list of errors when a batch containing 32 reports has multiple errors" ignore {
      ...
      validationService("8888").xmlNodeValidation(invalidBatch.head) mustBe List(
        ...
        Error(ErrorCode.INVALID_XML_XSD,List("Invalid content was found starting with element 'IllegalElement'. One of " +
          "'{\"http://www.govtalk.gov.uk/LG/Valuebill\":EntryDateTime}' is expected.")),
        Error(ErrorCode.INVALID_XML_XSD,List("'0£' is not a valid value for 'integer'.")),
        Error(ErrorCode.INVALID_XML_XSD,List("The value '0£' of element 'TotalNNDRreportCount' is not valid.")),
        Error(ErrorCode.CHARACTER ,List("Trailer", "IllegalElement", "Some text")),
        Error(ErrorCode.CHARACTER,List("Trailer", "TotalNNDRreportCount", "0£")),
        Error(ErrorCode.CHARACTER,List("138161", "StreetDescription", "23 NeW ST")),
        Error(ErrorCode.INVALID_XML_XSD,List("Invalid content was found starting with element 'BadElement'. " +
          "One of '{\"http://www.govtalk.gov.uk/LG/Valuebill\":TypeOfTax}' is expected.")),
        Error(ErrorCode.CHARACTER,List("138156", "Town", "WIMBLETOWN£")),
        Error(ErrorCode.INVALID_XML_XSD,List("Invalid content was found starting with element 'ExistingEntries'. " +
        ...
        Error(ErrorCode.ONE_EXISTING,List("138159", "CR09")))
    }
  }
